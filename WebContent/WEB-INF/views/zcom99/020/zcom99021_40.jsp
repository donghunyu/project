...
<script type="text/javascript">
    ...
    function requestPage(pageNo) {
        ...
        var action = "<%= baseLoc %>/" + "<%= ActionConstants_Aprj.ACTION_ZCOM99021_40 %>";
     ...
     *  상세검색
     **************************************************************************/
    function requestDtl(serNum) {
        if ( !chkDtl() ) {
            return false;
        }

        document.getElementById("serNum").value = serNum;
        var action = "<%= baseLoc %>/" + "<%= ActionConstants_Aprj.ACTION_ZCOM99021_41 %>" + getParam();
    ...
    function requestSave() {
        ...
        var action = "<%= baseLoc %>/" + "<%= ActionConstants_Aprj.ACTION_ZCOM99021_00 %>";
     ...
     **************************************************************************/
    function chkSave() {
        if ( isEmptyObject(document.getElementById(Trim("menuAuthCd                    ")), Trim("메뉴 권한 코드                ")) ) return false;
        ...
        if ( document.getElementById("menuAuthCd").value == "2000" ) {
    ...
    function requestDel() {
        ...
        var action = "<%= baseLoc %>/" + "<%= ActionConstants_Aprj.ACTION_ZCOM99021_80 %>";
     ...
     **************************************************************************/
    function openMenu() {
        key  = document.getElementById(Trim("serNum   "));
        name = "메뉴권한";
        if ( Trim(key.value) == "" ) {
        ...
        var action = ctx + "/" + "<%= ActionConstants_Aprj.ACTION_ZCOM99022_40 %>" + getParam();
        var target = "ZCOM99022";
            ...
            <jsp:include page="/include/zcom/menu/naviMenu.jsp" />
                ...
                <div class="bodyarea">
                    ...
                    <div class="filter">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <strong>메뉴 권한 코드</strong>
                                <p>
                                <span class="select-box w-full">
                                <select name="srchMenuAuthCd"               id="srchMenuAuthCd"               class="form-control input-sm">
                                    <option value="">전체</option>
                                    <c:forEach var="code" items="${menuAuthCdList}" varStatus="status">
                                    <option value="${code.cd}" <c:if test="${code.cd == srchMenuAuthCd}">selected</c:if>>${code.cdNm}</option>
                                    </c:forEach>
                                </select>
                                </span>
                                </p>
                    ...
                    <table class="table list">
                        ...
                        <tr style="cursor:pointer;" onclick="requestDtl('<c:out value="${row.serNum}"/>');">
                            <td class="tx-center"><input type="checkbox" name="delKey" id="delKey" title="선택" onclick="selectAll(event);" value="<c:out value="${row.serNum}"/>"></td>
                            <td class="tx-center"><c:out value="${row.rowNum                        }"/></td>
                            <td class="tx-center"><c:out value="${row.serNum                        }"/></td>
                            <td class="tx-center"><c:out value="${row.menuAuthNm                    }"/></td>
                ...
                <div class="bodyarea">
		            ...
		            <div id="detailLayer">
		                <jsp:include page="./zcom99021_41.jsp" />
		            </div>
                    <div class="btnarea_bottom">
                        <div class="btn default" title="메뉴등록" onclick="openMenu()   ;return false;">메뉴등록</div>

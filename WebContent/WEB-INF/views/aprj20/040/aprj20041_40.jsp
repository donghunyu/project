...
<script type="text/javascript">
    ...
    function requestPage(pageNo) {
        ...
        var action = "<%= baseLoc %>/" + "<%= ActionConstants_Aprj.ACTION_APRJ20041_40 %>";
    ...
    function requestInit() {
        ...
        requestDtl(itmSerNum);
    }

    /**************************************************************************
     *  상세검색
     **************************************************************************/
    function requestDtl(itmSerNum) {
        if ( !chkDtl() ) {
            return false;
        }

        document.getElementById("itmSerNum").value = itmSerNum;
        var action = "<%= baseLoc %>/" + "<%= ActionConstants_Aprj.ACTION_APRJ20041_41 %>" + getParam();
    ...
    function requestSave() {
        ...
        var action = "<%= baseLoc %>/" + "<%= ActionConstants_Aprj.ACTION_APRJ20041_00 %>";
    ...
    function chkSave() {
        ...
        if ( isEmptyObject(document.getElementById(Trim("reqSerNum                     ")), Trim("요청 일련번호                 ")) ) return false;
        if ( isEmptyObject(document.getElementById(Trim("itmVal                        ")), Trim("항목 값                       ")) ) return false;
        if ( isEmptyObject(document.getElementById(Trim("itmCtt                        ")), Trim("항목 내용                     ")) ) return false;
    ...
    function requestDel() {
        ...
        var action = "<%= baseLoc %>/" + "<%= ActionConstants_Aprj.ACTION_APRJ20041_80 %>";
        ...
        <div class="bodyarea">
            <div class="filter">
                <div class="form-group">
                            ...
                            <div class="input-group">
                                <strong>요청 일련번호</strong>
                                <p>
                                <input type="text" name="reqSerNum"            id="reqSerNum"            value="<c:out value="${resultHeader.reqSerNum             }"/>" class="w-full" title="업무 명" readonly="readonly"/>
                                </p>
                            </div>
                            <div class="input-group">
                                <strong>요청 종류 명</strong>
                                <p>
                                <input type="text" name="reqTypNm"             id="reqTypNm"             value="<c:out value="${resultHeader.reqTypNm              }"/>" class="w-full" title="요청 종류 명" readonly="readonly"/>
                                </p>
                            </div>
                            <div class="input-group">
                                <strong>요청 내용</strong>
                                <p>
                                <textarea          name="reqCtt"                       id="reqCtt"                                                                                               title="요청 내용"                    maxlength="200"  class="w-full" onKeyUp="setMaxLength();" ><c:out value="${resultHeader.reqCtt}"/></textarea>
                                </p>
                            </div>
            ...
            <!--//filter-->
                    ...
                    <table class="table list">
                        ...
                        <tr style="cursor:pointer;" onclick="requestDtl('<c:out value="${row.itmSerNum}"/>');">
                            <td class="tx-center"><input type="checkbox" name="delKey" id="delKey" title="선택" onclick="selectAll(event);" value="<c:out value="${row.itmSerNum}"/>"></td>
                            <td class="tx-left"  ><c:out value="${status.count                      }"/></td>
                            <td class="tx-left"  ><c:out value="${row.itmSerNum                     }"/></td>
                            <td class="tx-left"  ><c:out value="${row.itmVal                        }"/></td>
                            <td class="tx-left"  ><c:out value="${row.itmCtt                        }"/></td>
                        </tr>
                        </c:forEach>
                        </tbody>
                    </table>
                ...
                <div class="bodyarea">
                    <h4>항목 상세정보</h4>
                    <div id="detailLayer">
                        <jsp:include page="./aprj20041_41.jsp" />
                    </div>
